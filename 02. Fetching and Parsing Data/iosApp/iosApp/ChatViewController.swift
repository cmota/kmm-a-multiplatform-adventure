...
extension ChatViewController
{
    private func loadMessages() {
        Firestore.firestore().collection(collection).addSnapshotListener { snapshot, error in
            ...
            guard let snapshot = snapshot else {
                Gutenberg().e(tag: TAG, message: "Error fetching document: \(error!)")
                return
            }
            ...
            for document in snapshot.documents {
                ...
                Gutenberg().d(tag: TAG, message: "user=\(document.data()["username"]!) with message: \(document.data()["content"]!)")
            }
    ...
    private func sendMessage(message: String) {
        ...
        let message = ["username": PlatformDeviceKt.deviceName(),
                       "content": message,
